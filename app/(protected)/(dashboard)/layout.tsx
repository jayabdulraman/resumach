...
import { getPaidPackages, getUser, signOutAction } from "../../actions";
...
export default function DashboardNavbar({
  ...
  const getLayoutData = async () => {
    const [user, credit_packages] = await Promise.all([
      getUser(),
      getPaidPackages()
    ]);
    // @ts-ignore
          ...
          <Sheet>
            ...
            <SheetContent side="left" className="flex flex-col">
              <nav className="grid gap-2 text-lg font-medium">
                ...
                >
                  <span className="sr-only">resumach</span>
                </Link>
                <Link
                  href="/dashboard"
                  className={`flex items-center gap-3 rounded-lg px-3 py-2 transition-all hover:text-primary ${
          ...
          <div className="flex items-center ml-auto gap-4">
            {userData && (
              <div className="font-normal text-sm transition-all">
                Hey, {userData?.user_metadata.name ? userData?.user_metadata.name : userData?.email?.split("@")[0]}!
              </div>
            )}
