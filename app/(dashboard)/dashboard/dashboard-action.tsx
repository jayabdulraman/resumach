...
export function DashboardView({ UploadedUserFiles, CustomizedUserFiles, user, rateLimit, userProfile}: FileInterface) {
  ...
  return (
    <div className="flex flex-col gap-4 p-4 lg:gap-6 lg:p-6">
      <header>
        <h2 className="text-3xl font-bold">Customize resume</h2>
      </header>
          ...
          <CardFooter className="text-sm text-center border-t pt-5">
            {remaining > 0 ? ( // or if user used all credits (for pro users)
              ...
              <p className="text-red-600">
                You can only customize 5 resumes in 24 hours. Try again on {resetAt && format(resetAt, 'MMM dd, h:mm a')} or Upgrade!
              </p>
            )}
          </CardFooter>
        </Card>
      ...
      <div className="container mx-auto">
        <Tabs defaultValue="customized">
          <TabsList>
            <TabsTrigger value="customized">Customized resumes</TabsTrigger>
